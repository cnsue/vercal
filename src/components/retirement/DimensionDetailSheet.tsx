...
export default function DimensionDetailSheet({ dim, onClose }: Props) {
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 16, marginBottom: 16 }}>
          <div style={{ background, borderRadius: 12, padding: 14 }}>
            <DetailRing ratio={dim.ratio} unset={unset} reached={reached} />
          </div>
          <div style={{ flex: 1 }}>
...
function DetailRing({ ratio, unset, reached, size = 76 }: {
  ratio: number; unset: boolean; reached: boolean; size?: number
}) {
  const stroke = 6
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{
        ...
        color: '#fff', fontSize: 18, fontWeight: 900, letterSpacing: '-0.03em',
      }}>
        {display}
      </div>
    </div>
