...
const SYSTEM_PROMPT = [
  ...
  '',
  '盘中实时价 vs 收盘价的判别（非常重要，模型经常搞错）：',
  '- 行情页面上**同时显示**涨跌额（如 -0.18）和涨跌幅（如 -0.48%）的数字，是「盘中实时价」或「最近一次成交价」',
  '  → priceAsOf 必须填**今天**（如果今天是交易日）；如果今天是周末/节假日，则你查到的就是上个交易日的收盘价，priceAsOf 应填上个交易日',
  '  → 不要把盘中实时价写成「X 月 X 日收盘」；如果今天是 2026-05-18 周一，盘中价就是 2026-05-18 的，不是 2026-05-17（周日不开盘）也不是 2026-05-15（周五，那是上一个收盘日，价格不同）',
  '- 行情页面写明「X 月 X 日收盘」或显示"昨收"字样的，是历史收盘价 → priceAsOf 填那个日期',
  '- 盘中实时价标 confidence: "medium" 并在 sourceNote 注明「盘中实时价，非收盘」',
  '- 历史收盘价（最近交易日已经收盘）标 confidence: "high"，sourceNote 注明「X 月 X 日收盘」',
  '',
  ...
  '}',
  '',
  'sourceNote 必须包含两段逐字摘录（缺一不可，否则置信度只能填 low）：',
  '  a) 价格文本：例如「37.47 -0.18 -0.48%」或「昨收 37.65」',
  '  b) 页面上的行情时间戳：例如「行情时间 2026-05-18 11:30」或「2026-05-18 14:55:30 更新」',
  '     - 如果页面**没有**显式时间戳，必须明文写「页面未显示时间戳」',
  '     - 不要把网址中的日期、URL 参数、搜索结果摘要中的爬取时间当作行情时间戳',
  '',
  'confidence 评级标准（必须按此选择）：',
  '- high：sourceNote 同时含「价格文本」+「页面行情时间戳」，且时间戳与 priceAsOf 一致',
  '- medium：sourceNote 含价格文本但页面未显示时间戳（写了"页面未显示时间戳"），或者只是盘中实时价',
  '- low：无法摘录页面原文；任何用旧记忆补全的数据都必须标 low',
  '',
  '严禁：',
